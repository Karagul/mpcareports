...
```{r setup, include=F}
# This R chunk sets the options for all R chunks in the document
knitr::opts_chunk$set(message = F, warning = F, comment = "", eval = T, tidy = T)

```



```{r install-first, eval=F}
# Read me!
...
Here's an example showing an image of a turtle image that is saved locally in the same folder as the `.Rmd` file:



```
![Runaway Turtle](turtle.png)
```
...
>
> To size images in PDF documents, you can add `{out.width="25%"}` to an R chunk options. You then use the `knitr::include_graphics()` function to insert an image with the designated width.
>
> Here's how to show 3 tiny turtles by setting the image width to only extend 25% across the available space:
> 
...
include_graphics(c("turtle.png", "turtle.png", "turtle.png"))
```
